...
import { awaitLifecycleTermination, awaitTmuxTermination, closeTmux, closeWithPublicSdkProof, exitAndObservePostCloseFailure, exitTmux } from "./gjc-release-compat-lifecycle";
import { branchEntryId, openSessionDashboard, promptAndAwaitTerminal, rediscoverSessionId, sessionFromFilesystem, sessionIdFrom, sessionIdFromEndpoint, validateCurrentModel } from "./gjc-release-compat-runtime";
...
const cliEnvironment = ["env", "-i", `PATH=${process.env.PATH ?? ""}`, `TERM=${process.env.TERM ?? "xterm-256color"}`, `HOME=${root}`, `GJC_CODING_AGENT_DIR=${join(root, ".gjc", "agent")}`, `GJC_COMPAT_LOCAL_API_KEY=${apiKey}`, "NO_PROXY=*"];
...
try {
	...
	const initialTranscriptTurn = await promptAndAwaitTerminal(client, sessionId, "initial-transcript.turn.prompt", "Reply with initial-transcript-proof.", observe);
	...
	client = successor.client; sessionId = successor.sessionId;
	const createdSessionId = sessionId;
	if (createdSessionId === initialSession.sessionId) throw new Error("session.new did not produce a uniquely fresh session identity");
	const createdTranscriptTurn = await promptAndAwaitTerminal(client, sessionId, "created-transcript.turn.prompt", "Reply with created-transcript-proof.", observe);
	...
	observed.transcripts = { initial: { path: initialSession.transcript, sessionId: initialSession.sessionId, headerSessionId: initialSession.headerSessionId }, created: { path: createdTranscript, sessionId: createdSession.sessionId, headerSessionId: createdSession.headerSessionId } };
	successor = await lifecycleSuccessor(client, "session.resume", { id: initialSession.transcript }, workspace, observe, record, initialSession.sessionId);
	client = successor.client; sessionId = successor.sessionId;
	successor = await lifecycleSuccessor(client, "session.switch", { id: createdTranscript }, workspace, observe, record, createdSession.sessionId);
	client = successor.client; sessionId = successor.sessionId;
	const turn = await promptAndAwaitTerminal(client, sessionId, "turn.prompt", "Reply with compatibility-ok.", observe);
	await client.close(); client = await connectFor(workspace, sessionId);
	...
	await client.close().catch(() => undefined); client = await connectFor(workspace, branchSessionId);
	...
	if (createdSession.sessionId !== sessionId) throw new Error("created session transcript did not match the SDK endpoint");
	...
	const resumedTurn = await promptAndAwaitTerminal(client, resumedSessionId, "resume.turn.prompt", "Reply with post-resume-write-proof.", observe);
	await observe("Q17", () => client!.query("session.last_assistant"));
	observed.terminals = { initialTranscript: initialTranscriptTurn.terminal, createdTranscript: createdTranscriptTurn.terminal, createdSession: turn.terminal, resumed: resumedTurn.terminal };
	...
	observed.cleanup = { forcedTmuxSessions: [] }; observed.finishedAt = new Date().toISOString();
	...
	observed.cleanup = publicCloseInvoked
		? { forcedTmuxSessions: [], postClose: await exitAndObservePostCloseFailure(workspace, sdkLogicalClose?.endpoint, tmuxSession, run) }
		: { forcedTmuxSessions: await closeTmux(tmuxSession, run) };
	observed.finishedAt = new Date().toISOString(); await writeReports(root, observed); throw error;
} finally { await client?.close(); await provider.stop(); }
...
async function observe(name: string, action: () => Promise<unknown>): Promise<unknown> {
	...
	(observed.operations as Array<Record<string, unknown>>).push({ name, shape: shapeOf(value), observed: value }); return value;
}
function record(name: string, value: unknown): void { (observed.operations as Array<Record<string, unknown>>).push({ name, observed: value }); }
function shapeOf(value: unknown): unknown { return Array.isArray(value) ? value.map(shapeOf) : isRecord(value) ? Object.fromEntries(Object.entries(value).map(([key, child]) => [key, shapeOf(child)])) : typeof value; }
function isRecord(value: unknown): value is Record<string, unknown> { return value !== null && typeof value === "object" && !Array.isArray(value); }
function startupArguments(version: string): readonly string[] { const arguments_ = ["--model", "compat-local/hermetic-model"]; return version === "0.11.1" ? arguments_ : [...arguments_, "--thinking", "off"]; }
async function releasedCliVersion(command: string): Promise<string> { const output = (await run(command, ["--version"])).trim(); const match = /^(?:gjc\/)?(\d+\.\d+\.\d+)$/.exec(output); if (match === null) throw new Error(`gjc --version returned an invalid version: ${JSON.stringify(output)}`); return match[1]; }
async function run(command: string, args: readonly string[], env?: Record<string, string | undefined>, allowFailure = false): Promise<string> {
	const process = Bun.spawn([command, ...args], { env, stdout: "pipe", stderr: "pipe" }); const [code, stdout, stderr] = await Promise.all([process.exited, new Response(process.stdout).text(), new Response(process.stderr).text()]);
	if (code !== 0 && !allowFailure) throw new Error(`${command} ${args.join(" ")} failed: ${stderr.trim()}`); return stdout;
}
