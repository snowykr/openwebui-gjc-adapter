...
- Preserved the strong gate-answer hash binding on replays of a still-current workflow gate operation (conflicting payloads are rejected again), while replays of gate operations superseded by a regular turn keep the durable detail binding, and replayed control operations re-enqueue projection rows only when still current and only from the record mapping so payload hashes match completion-time rows.
