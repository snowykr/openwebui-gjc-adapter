...
- Hardened the WAL durability semantics: failures after a mutation is durably committed (WAL append or base rename) surface as `SessionAuthorityDurabilityError` with the WAL-visible state retained instead of rolling back memory, so a retry cannot conflict with or duplicate a committed operation; unverified WAL rollbacks reload the replayable state; the WAL truncation is fsynced before a rollback is reported; deleting a failed new WAL syncs its directory entry; the WAL base binding now includes a per-rewrite document generation so timestamp-preserving external replacements cannot replay stale deltas (the live per-mutation verification re-checks that generation with a layout-independent streaming scan that also tolerates external whitespace formatting); failed post-commit reloads keep the durability-error classification; WAL replay and rollback seed from internal references instead of deep-copying the whole authority; and compaction rewrites the in-memory journal to the normalized records so stripped event arrays are not re-introduced by later WAL deltas.
