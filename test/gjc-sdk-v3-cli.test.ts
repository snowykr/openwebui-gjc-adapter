...
describe("endpoint-less CLI lifecycle boundary", () => {
	test("creates a tagged CLI session and discovers its id through /session", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({
			cliPath: "/opt/gjc",
			cwd: sessionRoot,
			childEnvironment: { HOME: "/runtime-home", GJC_CONFIG_DIR: ".gjc", GJC_CODING_AGENT_DIR: "/runtime-home/.gjc/agent" },
			tmux,
		});
	...
	test("sends /session exactly once across capture rollover and rejects ambiguous fresh captures", async () => withSessionRoot(async sessionRoot => {
	...
	test("resumes with absolute JSONL and its parent session directory", async () => withSessionRoot(async sessionRoot => {
		...
		expect(tmux.calls[0]?.at(-1)).toBe(`'env' '/opt/gjc' '--resume' '${sessionPath}' '--session-dir' '${sessionRoot}'`);
	}));
	test("rejects a resume path replaced by rename after its descriptor is opened", async () => withSessionRoot(async sessionRoot => {
		const sessionPath = join(sessionRoot, "session-1.jsonl");
		...
		const backend = new CliLifecycleBackend({
			cliPath: "/opt/gjc",
			cwd: sessionRoot,
			tmux: new FakeTmuxRunner(sessionRoot, async () => { await rename(replacement, sessionPath); }),
		});
	...
	test("rejects a resume path replaced by symlink after its descriptor is opened", async () => withSessionRoot(async sessionRoot => {
		const sessionPath = join(sessionRoot, "session-1.jsonl");
		const replacement = join(sessionRoot, "replacement.jsonl");
		await writeFile(sessionPath, sessionJsonl("session-1", sessionRoot));
		await writeFile(replacement, sessionJsonl("session-1", sessionRoot));
	...
	test("accepts an owned CLI exit request without treating it as a public close acknowledgement", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({ cliPath: "/opt/gjc", cwd: sessionRoot, tmux });
	...
	test("uses /exit then proves closure after a public SDK acknowledgement without post-ack kill fallback", async () => {
		...
		await withSessionRoot(async sessionRoot => {
			...
			await expect(unresolved.requestExitAndProveClosedAfterAcknowledgement(unresolvedCreated.value, 1)).resolves.toMatchObject({
				status: "uncertain",
			});
			expect(unresolvedTmux.calls).toContainEqual(["send-keys", "-t", unresolvedCreated.value.pane.target, "/exit", "Enter"]);
			expect(unresolvedTmux.calls.some(call => call[0] === "kill-pane")).toBe(false);
		});
	});
	test("proves idempotent close after acknowledgement when tmux has already vanished", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({
			cliPath: "/opt/gjc",
			cwd: sessionRoot,
	...
	test("does not treat vanished tmux as closure proof while the endpoint or original PID remains live", async () => {
		await withSessionRoot(async sessionRoot => {
			...
			await expect(backend.requestExitAndProveClosedAfterAcknowledgement(created.value, 1)).resolves.toMatchObject({
				status: "uncertain",
				message: expect.stringMatching(/server exited unexpectedly; closure proof is uncertain: session endpoint is still available/),
			});
			expect(tmux.calls.some(call => call[0] === "kill-pane")).toBe(false);
		});
		await withSessionRoot(async sessionRoot => {
	...
	test("uses CLI cleanup only before a public close acknowledgement and only for its owned pane", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({ cliPath: "/opt/gjc", cwd: sessionRoot, tmux });
		const created = await backend.create({ sessionRoot });
		if (created.status !== "closed") throw new TypeError("fixture must create an attachment");

		expect(await backend.fallbackBeforeCloseAcknowledgement(created.value)).toEqual({ status: "closed", value: undefined });
		expect(tmux.calls).toContainEqual(["kill-pane", "-t", created.value.pane.target]);
	}));
	test("proves close after endpoint disappearance when the private tmux server cannot report its pane", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({
			cliPath: "/opt/gjc",
			cwd: sessionRoot,
	...
	test("times out uncertain when the original PID remains alive after acknowledgement", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({
			cliPath: "/opt/gjc",
			cwd: sessionRoot,
	...
	test("requires endpoint disappearance and distinguishes liveness-query unavailability", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({
			cliPath: "/opt/gjc",
			cwd: sessionRoot,
		...
		const unavailable = new CliLifecycleBackend({
			...
			cwd: sessionRoot,
			tmux,
			isProcessAlive: () => { throw new Error("process table unavailable"); },
		});
	...
	test("does not kill an owned pane after a close acknowledgement when closure remains unproven", async () => withSessionRoot(async sessionRoot => {
		const tmux = new FakeTmuxRunner(sessionRoot);
		const backend = new CliLifecycleBackend({ cliPath: "/opt/gjc", cwd: sessionRoot, tmux });
		const created = await backend.create({ sessionRoot });
		if (created.status !== "closed") throw new TypeError("fixture must create an attachment");
...
class FakeTmuxRunner implements TmuxCommandRunner {
	...
	constructor(
		...
		private readonly behavior: { readonly rollover?: boolean; readonly ambiguous?: boolean; readonly exitClosesSession?: boolean } = {},
	) {}
	...
	async run(argv: readonly string[]): Promise<TmuxCommandResult> {
		...
		switch (argv[0]) {
			case "new-session": {
				...
				await writeFile(
					join(this.sessionRoot, "session-1.jsonl"),
					sessionJsonl("session-1", cwd),
				);
			...
			case "display-message":
				if (argv.at(-1) === "#{history_size}|#{cursor_y}") {
					return { exitCode: 0, stdout: this.sessionRequested ? this.behavior.ambiguous ? "0|3\n" : "0|2\n" : "0|1\n", stderr: "" };
				}
				if (argv.at(-1) === "#{pane_dead}|#{pane_pid}|#{pane_current_command}") {
					return { exitCode: 0, stdout: "0|123|gjc\n", stderr: "" };
				}
				switch (this.ownershipState) {
					case "absent": return { exitCode: 1, stdout: "", stderr: "can't find pane" };
					case "replaced": return { exitCode: 0, stdout: `%4|124|${this.ownershipTag}\n`, stderr: "" };
					case "uncertain": return { exitCode: 0, stdout: "", stderr: "" };
					case "unavailable": return { exitCode: 1, stdout: "", stderr: "server exited unexpectedly" };
					case "normal": return this.exists
					...
					stdout: this.sessionRequested
						? this.behavior.rollover
							...
							: this.behavior.ambiguous
								...
								: "ready\nSession ID: session-1\n"
						: this.behavior.rollover ? "old screen\n" : "ready\n",
					stderr: "",
				};
			case "kill-pane":
				this.exists = false;
