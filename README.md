...
Lifecycle certainty labels and error names are not public nonapplication receipts. The journal rejects post-invocation terminal classification, restoration using retained pre-close proof, and indirect retries through uncertain/acknowledged cleanup states. Local pre-invocation failure and initial cleanup of an acknowledged target remain distinct. These fail-closed guards do not implement the unavailable public recovery proof. Runtime shutdown also uses one monotonic budget: graceful drain receives at most half, reserving the remainder for local Router stop; exhaustion cannot dispatch a later stop or certify shutdown.
...
Branch receipts likewise bind to the immutable admitted source: a same-tenant predecessor replacement during fork cannot erase the returned successor identity. The canonical operation retains its acknowledgement and successor before rejecting renewed predecessor currentness; no adoption, prompt, or publication follows, and reopen preserves uncertainty without another fork. Pre-invocation replacement still prevents dispatch. This receipt retention is not original-incarnation recovery or cleanup authorization.

Successor creation shares a finite budget across source admission, invocation and acknowledgement callbacks, fork, target proof, publication callback, and cleanup. The runner passes its configured turn timeout; remaining time reaches compound runtime proof calls, and expired queued reconciliation cannot dispatch later. Expiration does not start cleanup with a fresh timeout or discard the acknowledged target. This bounds the successor flow, not the entire subsequent branch prompt, and does not supply the missing public exact-close or crash-recovery contract.
...
Model catalog queries use the same complete-page collector as live queries: incomplete pages need a nonrepeating continuation cursor, with 256-page and 100,000-item limits. One query budget covers lease checks, requests, all pages, and failure cleanup; a stopped reader rejects new or late results, and repeated stop calls retain the original shutdown failure. Temporary catalog provisioning still lacks the full durable lifecycle-owner and public exact-close/recovery contract; these query fixes do not enable it.

`--bootstrap` exercises the restricted adapter coordinator against the real public SDK and an owned durable workspace lease. For already tenant-scoped historical mappings, it reopens ordinary V3 staging before creating a Router, selects the exact saved session publicly, persists one manifest/occurrence-bound intent before resume, retains acknowledgement before adoption checks, and commits that same staged journal without rewriting old results. A final synchronous attachment/lease check precedes replacement. The bootstrap Router stops before the coordinator returns; it never supplies serving capabilities. Unscoped ownership, unresolved provisionals/reassignments, and interrupted incarnation recovery remain blocked. This internal coordinator is not yet an operator provisioning command and does not enable V2 serving startup. The obsolete flat-bootstrap store and server hook have been removed; canonical V3 alone owns the staged graph and lifecycle journal.
