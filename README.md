...
`--bootstrap` exercises the restricted adapter coordinator against the real public SDK and an owned durable workspace lease. For already tenant-scoped historical mappings, it reopens ordinary V3 staging before creating a Router, persists one manifest/occurrence-bound intent, selects the exact saved session publicly, retains acknowledgement before adoption checks, and commits that same staged journal without rewriting old results. A final synchronous attachment/lease check precedes replacement. The bootstrap Router stops before the coordinator returns; it never supplies serving capabilities. Unscoped ownership, unresolved provisionals/reassignments, and interrupted incarnation recovery remain blocked. This internal coordinator is not yet an operator provisioning command and does not enable V2 serving startup. Legacy flat-bootstrap modules still require removal after their behavioral coverage is transferred.
