...
async function loadProvenGjcSessionFile(sessionRoot: string, filePath: string): Promise<LoadedGjcSessionFile> {
	...
	try {
		...
		if (!held.isFile()) {
			throw new GjcSessionLoadError(candidate, [
				{
					code: "corrupt_session_file",
					message: `GJC session candidate is not a regular file: ${candidate}`,
					filePath: candidate,
				},
			]);
		}
		if (held.size > MAX_DISCOVERY_SESSION_BYTES) {
			throw new GjcSessionLoadError(candidate, [
				{
					code: "corrupt_session_file",
					message: `GJC session candidate exceeds the discovery size limit: ${candidate}`,
					filePath: candidate,
				},
			]);
		}
		...
		if (!current.isFile() || current.dev !== held.dev || current.ino !== held.ino) {
			throw new GjcSessionLoadError(candidate, [
				{
					code: "corrupt_session_file",
					message: `GJC session candidate changed while loading: ${candidate}`,
					filePath: candidate,
				},
			]);
		}
		...
		throw new GjcSessionLoadError(
			...
			[
				{
					code: "corrupt_session_file",
					message: `Cannot safely load GJC session file ${candidate}`,
					filePath: candidate,
				},
			],
			error,
	...
	for (let index = 0; index <= bytes.length; index++) {
		...
		if (line.length > MAX_DISCOVERY_SESSION_LINE_BYTES) {
			throw new GjcSessionLoadError(filePath, [
				{
					code: "corrupt_session_file",
					message: `GJC session line exceeds the discovery size limit: ${filePath}`,
					filePath,
				},
			]);
		}
		...
		if (
			...
			throw new GjcSessionLoadError(filePath, [
				{
					code: "corrupt_session_file",
					message: `GJC session file contains an invalid entry: ${filePath}`,
					filePath,
				},
			]);
		}
...
export async function loadGjcSessionFile(filePath: string): Promise<LoadedGjcSessionFile> {
	try {
		...
		throw new GjcSessionLoadError(
			filePath,
			[{ code: "corrupt_session_file", message: `Failed to load GJC session file ${filePath}`, filePath }],
			error,
		);
	}
}
