...
export class SessionMappingStore {
	...
	operation(chatId: string, operationId: string): SessionOperation | undefined {
		return this.#operations
			.get(chatId)
			?.find(operation => operation.id === operationId || operation.ingressId === operationId);
	}
	beginOperation(chatId: string, operation: Omit<SessionOperation, "state" | "startedAt" | "completedAt">): void {
		...
		if (operations.some(candidate => candidate.id === operation.id || candidate.ingressId === operation.ingressId))
			return;
		this.#operations.set(chatId, [
			...operations,
			{ ...operation, state: "pending", startedAt: new Date().toISOString() },
		]);
	}
	transitionOperation(
		chatId: string,
		...
		const operations = this.#operations.get(chatId);
		const index =
			operations?.findIndex(operation => operation.id === operationId || operation.ingressId === operationId) ?? -1;
	...
	reserveProvisionalOperation(
		...
		if (prior !== undefined) {
			if (
				...
				throw new Error(
					`GJC operation ${operation.ingressId ?? operation.id} conflicts with a different ingress payload.`,
				);
	...
	publishProvisionalOperation(
		...
		this.#operations.set(operation.chatId, [
			...operations,
			{
				id: operation.id,
				kind: "prompt",
				...
				result: operationResult("turn", published),
			},
		]);
	...
	attachProvisionalOperation(
		...
		if (
			...
			attachment.sessionFile === undefined ||
			attachment.attachment === undefined
		)
			throw new Error(`GJC operation ${ingressId} requires reconciliation.`);
...
export class FileBackedSessionMappingStore extends SessionMappingStore {
	...
	constructor(filePath: string) {
		super();
		this.#authority = new FileSessionAuthority(filePath);
	}
	override get(chatId: string): SessionMapping | undefined {
		const mapping = this.#authority.get(chatId);
		return mapping === undefined ? undefined : copySessionMapping(mapping);
	}
	override set(mapping: SessionMapping): SessionMapping {
		return copySessionMapping(this.#authority.set(copySessionMapping(mapping)));
	}
	override upsert(mapping: SessionMapping): SessionMapping {
		return copySessionMapping(this.#authority.upsert(copySessionMapping(mapping)));
	}
	override entries(): readonly SessionMapping[] {
		return this.#authority.entries().map(copySessionMapping);
	}
	override operation(chatId: string, operationId: string): SessionOperation | undefined {
	...
	override completeOperationWithMapping(
		...
		return copySessionMapping(
			this.#authority.completeOperationWithMapping(
				...
				operationResult(kind, mapping),
			),
		);
	}
	override provisionalOperation(chatId: string, ingressId: string): ProvisionalSessionOperation | undefined {
	...
	override publishProvisionalOperation(
		...
		return copySessionMapping(this.#authority.publishProvisionalOperation(operation, mapping));
	}
...
export function copySessionMapping(mapping: SessionMapping): SessionMapping {
	return {
		...
		...(normalizeModelSelection(mapping.modelSelection) === undefined
			? {}
...
function provisionalKey(chatId: string, ingressId: string): string {
