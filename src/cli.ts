...
import {
	...
	parseBootstrapState,
	resetBootstrapState,
} from "./configure/bootstrap-state";
import {
	canDisplaySecret,
	displayAdapterToken,
	generateAdapterToken,
	readSecretRecordFromFd,
} from "./configure/credentials";
import { CliUsageError, parseCliArguments } from "./configure/grammar";
import {
	type CommandRunner,
	checkManagedComposePrerequisites,
	managedAdapterImagePlan,
	renderManagedCompose,
} from "./configure/managed-compose";
import { configureOpenWebUI } from "./configure/openwebui-setup";
import { runPhaseAwareDeployment } from "./configure/orchestrator";
import {
	acquireConfigLock,
	acquireRouteLock,
	canonicalizeUrl,
...
import { renderExistingSystemdUnit, renderSystemdComposeUnit, routeControllerUnitName } from "./configure/systemd";
import { registerProjectDirectory } from "./projects/registry";
...
export interface CliDependencies {
	...
	readonly startServer?: (config: ReturnType<typeof loadInstalledAdapterConfig>) => AdapterServerHandle;
		...
		if (config.mode === "managed") {
			...
			writeFileSync(
				...
				renderExistingSystemdUnit({
					...
					adapterCommand: `${process.execPath} ${process.argv[1]} serve --config ${path}`,
				}),
				{ mode: 0o600 },
			);
			mkdirSync(userUnitDirectory, { recursive: true, mode: 0o700 });
...
export async function runCli(
	...
	try {
		...
		if (command.kind === "configure") {
			...
			const unlockRoute = acquireRouteLock(path);
			try {
				await configure(
					command.mode,
					options,
		...
		const server = dependencies.startServer
			? dependencies.startServer(config)
			: startAdapterServer(await buildProductionServerOptions(config));
...
async function buildProductionServerOptions(
	config: ReturnType<typeof loadInstalledAdapterConfig>,
): Promise<AdapterServerOptions> {
	const { createGjcRpcTurnRunner } = await import("./gjc/rpc-runner");
	const { FileBackedSessionMappingStore } = await import("./gjc/session-router");
	const { createGjcRoutingLiveGatewayRunner } = await import("./live/gjc-routing-runner");
	const allowedRoots = await resolveAllowedRoots(config.allowedProjectRoots);
	const project = await registerProjectDirectory(
		{ cwd: config.allowedProjectRoots[0], name: "default", sessionRoot: config.sessionRoot },
		allowedRoots,
	);
	const owner = { ownerUserId: config.ownerUserId ?? config.installationId ?? "", singleOwnerLocalMode: true };
	const mappings = new FileBackedSessionMappingStore(join(config.sessionRoot, "openwebui-session-mappings.json"));
	const turnRunner = createGjcRpcTurnRunner({ cliPath: config.gjcCommand });
	return {
