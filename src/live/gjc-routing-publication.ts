...
export async function publishControlMapping(mappings: SessionMappingStore, lifecycle: GjcLifecycleTransaction, turn: LiveGatewayRunnerInput, existing: SessionMapping, applied: GjcControlResult, hash: string, afterPublish: (mapping: SessionMapping) => void): Promise<SessionMapping> {
	...
	return lifecycle.publish(proof, () => {
		const published = mappings.completeOperationWithMapping(turn.chatId, turn.userMessageId, hash, {
			...existing,
			...
			attachment: proof,
			...(applied.result === undefined ? {} : { rawFrameCursor: applied.result.rawFrameCursor, eventCursor: applied.result.eventCursor, events: applied.result.events }),
		}, "control");
...
export function controlOperationKind(operation: NonNullable<LiveGatewayRunnerInput["control"]>["operation"]): "branch" | "reply" | "gate" | "prompt" {
...
export function controlOperationHash(turn: LiveGatewayRunnerInput): string {
	return createHash("sha256").update(JSON.stringify({ chatId: turn.chatId, projectId: turn.project.id, parentId: turn.userMessageParentId, prompt: turn.prompt, control: turn.control })).digest("hex");
}
