...
import {
	...
	replayWithLifecyclePublication,
		...
		async run(turn: LiveGatewayRunnerInput): Promise<GjcRoutingLiveGatewayRunnerResult> {
			...
			if (turn.control !== undefined && priorOperation?.state === "complete") {
				...
				if (
					...
					throw new Error(
						`GJC operation ${turn.userMessageId} completed without a valid immutable result binding.`,
					);
			...
			if (
				turn.control !== undefined &&
				(priorOperation?.state === "uncertain" || priorOperation?.state === "conflict")
			)
				...
				const predecessor = await controlled.withLifecyclePublication(
					...
					async lifecycle => {
						...
						try {
							...
							return {
								applied,
								mapping: await publishControlMapping(
									input.mappings,
									lifecycle,
									turn,
									existing,
								...
								return withCanonicalModel(
									{
										...
										...(result.events.length === 0
											? {}
											: { events: projectTurnEvents(result.events, undefined) }),
									},
