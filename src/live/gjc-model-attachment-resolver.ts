...
export function createPublicSdkModelAttachmentResolver(input: {
	...
	return async () => {
		...
		const backend = new CliLifecycleBackend({ cliPath: input.cliPath, cwd: input.cwd, childEnvironment: input.childEnvironment });
		...
		try {
			...
			return registerTemporaryModelAttachment(attachment, async port => {
				...
				try {
					...
					const closed = await backend.requestExitAndProveClosedAfterAcknowledgement(lifecycle, MAX_LIFECYCLE_CLOSE_PROOF_WINDOW_MS);
					if (closed.status !== "closed") throw new Error(`temporary model session close is ${closed.status}: ${closed.message}`);
					...
					if (fallback.status !== "closed") throw new AggregateError([error, new Error(fallback.message)], "temporary model session cleanup is uncertain");
					input.onProvenClosed?.(resolve(input.cwd), lifecycle.sessionId);
				}
			});
		} catch (error) {
			const fallback = await backend.fallbackBeforeCloseAcknowledgement(lifecycle);
			if (fallback.status !== "closed") throw new AggregateError([error, new Error(fallback.message)], "temporary model session endpoint cleanup is uncertain");
			input.onProvenClosed?.(resolve(input.cwd), lifecycle.sessionId);
			throw error;
		}
	};
